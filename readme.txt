...
потребуются иные функции, то есть Lua-скрипты и встроенная поддержка языка
...
При запуске сборки автоматически выставляется размер окна 80x35 и шрифт Lucida
Console,15 (15pt). Если вы указали запуск в полноэкранном режиме консоли, то
будет выставлен только шрифт, а размер окна не изменится. Это обеспечивается
Lua-скриптом "Common_ConsControl.lua", в который встроена необходимая
автоматика. Поменять значения по умолчанию вы можете, отредактировав скрипт в
строках 15 - 17 и 21, не трогая команды плагина:
...
Минимальные системные требования:
 ...
 Windows XP/2003 SP3 и выше (на ядре Windows 5.1/5.2 функциональность набора
 ограничена WinAPI), примерно 150 Мб места на диске, пользователь должен иметь
 права записи в каталог данной копии Far3 (настройки хранятся в локальной БД),
 процессор Intel Pentium D/AMD64 или аналогичный с наборами инструкций SSE2/3,
 ("спасибы" в Микрософт за VC++201х - он эти команды молча в код вставляет),
 для запуска требуется до 100 Мб оперативной памяти.
 ...
 OpenSSL Toolkit v1.1.1l и новее.
...
В случае сбоя профиля, что видно по проблемам с запуском или при выходе из
Far попробуйте сбросить его настройки с помощью скрипта recover.cmd
расположенном в каталоге куда установлена сборка.
...
A: На самом деле ошибки нет, Far правильно выводит размер, но если наш каталог
   содержит жёсткие и/или символьные ссылки, либо является точкой
   монтирования, то у вас может сложиться впечатление, что подсчёт ошибочен
   хотя ошибки нет, поскольку не все файлы которые вы видите в каталоге,
   реально находятся в нём - Файлы на которые указывают символьные/жёсткие
   ссылки, на деле находятся вне каталога, а в нём только числятся для
   удобства использования. Да и путаница аббревиатур даст ошибку в оценке т.к.
   в ВТ исторически используются системы счисления с основанием 2, и
   1Gb=1024^3=1073741824, но многие "округляют" его до 1 GiB=10^9=1000000000,
   и пишут 1Gb=1GiB вроде и разница всего-то проценты с хвостиком, да от
   подмены понятий и смысл слегка меняется…
...
A: Это окно выводит утилита LMDataMove если у вас обнаружены старые пути к
   ...
   уверен в том, что в этом есть смысл, так не могу заранее знать какие именно
   версии скриптов у вас установлены. Утилита переносит настройки скриптов
   BookmarkManager, CtrlAltMenuDisk, EnvManager и LuaManager в новый подраздел
   БД LuaMacro. Я могу убрать её запрос, но это может привести к ошибкам и
   поэтому пока он оставлен, а после я планирую её саму убрать.
...
A: Плагины и скрипты не входящие в сборку пользователь обновляет сам,
...
Q: Архиваторы должны быть доступны в %PATH% или находится в каталоге Far.
...
Q: Скорость работы очень "высокая". Например дистрибутив GIMP 2.10.18 Rev 2
   на i7-2600 с 16 Gb DDR3-1600 9-9-9-24 Cr2 innounp.so v0.49.1 при условии
   остановки всех иных задач распаковывал более 49 часов, а оригинальный
   innounp/exe v0.49 чуть больше двух минут, и ничего останавливать не
   потребовалось.
...
A: Нет, это особенность геометрии экрана - изначально все консольные программы
   ...
   всего использовать UNICIODE шрифты которые в отличие от матричных шрифтов,
   легко масштабируются и позволяют отображать символы произвольных языков.
...
A: Изменить размер окна мы можем в любой момент - нажимаем левую кнопку мыши
   ...
   этом мы увидим текущий размер окна, а ЛКМ + колесо позволяет изменить
   размеры шрифта. Изменения действуют только на текущий сеанс, для их
   сохранения надо исправлять ярлык программы.
