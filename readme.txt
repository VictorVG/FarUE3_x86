...
Минимальные системные требования:
 ...
 ограничена WinAPI), примерно 80 Мб на диске, пользователь должен иметь права
 записи в каталог данной копии Far3 (настройки хранятся в локальной БД),
 ...
 для запуска требуется до 50 Мб оперативной памяти.

 Для работы модуля Observer::VDISK необходимо наличие установленного MS
 .NET Frimework 4.0 (иначе он не запустится), для иных плагинов .NET
 Framework не требуется.

 OpenSSL Toolkit v1.1.0f и новее.

 Perl v5.24 и новее для работы конверторов Markdown в BB-code и/или HTML.
...
обновления для обычной установки. Также в архиве может присутствовать модуль
farue3-addons.exe - силент инсталлер утилит cURL и Wget которые пришлось
выделить в отдельный элемент из-за ограничений Git. При установке он должен
находится рядом с основным установщиком farue3-setup.exe и после его
завершения будет автоматически запущен для их установки.

За это неудобство скажите "спасибо" админам YandexDisk которые под надуманным
...
Q: Входящий в сборку Wget выводит сообщения на английском языке. Почему?

A: Wget выводит сообщения на языке соответствующем текущей локали Far если
   его рабочий каталог совпадает с каталогом где находится сам Wget. Ключей
   для принудительного выбора локали у программы нет, по умолчанию Wget
   выводит сообщения на английском языке.
...
Q: В b4423 и старше - да, раньше - нет.
   ...
   Ограничение: UNC пути в стиле UNIX применимы только для команд OC реализованных
   в Far, панелей, вызовов far -e/-v т.е. в тех случаях когда не вызывается CMD.
...
Q: Или через плагин filever (хоткей CtrlAltV), но он не работает (падает) если
   ...
   v1.1.x). Для исполняемых файлов формата РЕ (Win32/64) ограничений нет. Плагин
   считывает информацию из структуры VERSION_INFO, но ЭЦП не проверяет. Это
   задача оболочки ОС. Также можно посмотреть версию файла через плагин
...
A: В Far и собираемых с ним плагинах вставка шпионских функций MS VC++2015
   заблокирована на уровне исходников.
